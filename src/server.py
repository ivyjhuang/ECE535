...
class Server:
    ...
    def eval(self, data_test):
        ...
        arrayClasses, countsPerClass = np.unique(y_samples, return_counts = True)

        # print("UH")
        # print(y_samples.flatten())
        ...
        for i in range(n_eval_process):
            ...
            new_array = y_samples.flatten()
       
            for k in equals:
                if k[0]: # if equals is true, it predicted jth index of lables
                    # classCount[new_array[j]] = classCount[new_array[j]] + 1
                    classCount[new_array[m]] =  classCount[new_array[m]] + 1
                    # print(new_array[i])
                    # print("true")
        ...
        classAccuracy= {}
        for i in hello.keys():
            classAccuracy[i] = (classCount[i]/hello[i])*100
